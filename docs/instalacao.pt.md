...
2. Pergunta quais **Teams** de agentes instalar. `Solucao Agents Core` está sempre incluído; `Migration Agents`, `Code Forward Agents` e `Pricing and Size Agents` já vêm marcados; `Translators N8N->Specs->Python` fica desmarcado por padrão. O CLI expande cada Team escolhido nos seus agentes
...
6. Cria a estrutura `.solucao/` com estado, configuração e plano
7. Gera o manifesto SHA-256 para atualizações seguras no futuro

É tipo um `npm install`, mas para o seu time de agentes de engenharia solucao.
...
```
projeto-legado/
├── .solucao/               ← estado, config e contexto da análise
...
└── _solucao_sdd/           ← onde as especificações serão geradas (vazio inicialmente)
```
...
!!! warning "Recomendação forte: faça um backup"
    Embora o Solucao nunca modifique seus arquivos, agentes de IA podem cometer erros. Antes de iniciar a análise:
