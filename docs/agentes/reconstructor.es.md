...
El Reconstructor convierte las specs del Solucao en un **plan de reconstrucción ejecutable** y luego implementa cada tarea bajo demanda, bottom-up.
...
| Archivo | Contenido |
|---------|-----------|
| `_solucao_sdd/reconstruction-plan.md` | Lista completa de tareas bottom-up con `Lee:` y `Listo cuando:` por tarea, más alertas pre-vuelo mapeadas desde `gaps.md` |
...
- Después de que `/solucao` terminó y quieres reimplementar el legado desde cero bajo las specs originales.
- Después de que `/solucao-migrate` terminó y quieres materializar el sistema nuevo a partir de las specs de la migración.
...
/solucao  →  specs (_solucao_sdd/)
                  │
                  ▼
/solucao-reconstructor  →  reconstruction-plan.md (una vez)  →  código, una tarea a la vez
```

Es **independiente del pipeline principal**: nunca bloquea `/solucao`, `/solucao-forward` ni `/solucao-migrate`. Se invoca por separado cuando eliges reconstruir.
