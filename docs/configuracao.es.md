...
Solucao guarda toda su configuración y estado del análisis dentro de la carpeta `.solucao/` en la raíz del proyecto.

---

## Estructura de la carpeta `.solucao/`
...
Define el volumen de artefactos que cada agente genera durante el análisis. **No se configura en la instalación:** Solucao lo pregunta al inicio del primer análisis, después de que el Scout mapea el proyecto, para que decidas con información real.
...
La elección se guarda en `.solucao/state.json` en el campo `doc_level`. Puedes editarlo manualmente en cualquier momento para ajustar el nivel durante un análisis en curso.
