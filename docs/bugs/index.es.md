...
Siempre instalado, como grupo único fijo junto al Solucao Agents Core.
...
El Equipo funciona mejor sobre una extracción (`_solucao_sdd/`), pero degrada bien: sin specs, los bugs llevan la etiqueta `spec-gap` hasta que corra `/solucao`.
...
Toda corrección responde: *¿lo incorrecto era el código o la spec?* Tres salidas: `spec-correta` (el código divergió, la spec queda), `spec-desatualizada` (se genera un addendum versionado en `_solucao_sdd/addenda/`, la spec original nunca se edita) o `spec-gap` (el comportamiento nunca fue especificado; nace un addendum aditivo). El agente recomienda con evidencias; **la decisión es humana**. El diff del código y el diff de la spec quedan registrados **juntos** en la Resolution del bug.

El vínculo inverso vive también del lado de la spec: `_solucao_sdd/traceability/bugs.md` es un espejo generado que lista, por sección de spec, los bugs que la afectan.

---

## Non-destructive

Los Bug Agents escriben solo dentro de `_solucao_bugs/`, además de los addenda en `_solucao_sdd/addenda/` y el espejo generado en `_solucao_sdd/traceability/`. El código del proyecto solo cambia por las dos puertas de aprobación, con diffs explícitos. Los bugs con `visibility: restricted` (seguridad) quedan fuera de las vistas públicas y nunca llegan a harness externos.
