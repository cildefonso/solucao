...
Sempre instalado, como grupo único fixo ao lado do Solucao Agents Core.
...
O Time funciona melhor sobre uma extração (`_solucao_sdd/`), mas degrada bem: sem specs, os bugs carregam o label `spec-gap` até o `/solucao` rodar.
...
Toda correção responde: *o errado era o código ou a spec?* Três saídas: `spec-correta` (o código divergiu, a spec fica), `spec-desatualizada` (um adendo versionado é gerado em `_solucao_sdd/addenda/`, a spec original nunca é editada) ou `spec-gap` (o comportamento nunca foi especificado; nasce um adendo aditivo). O agente recomenda com evidências; **a decisão é humana**. Diff do código e diff da spec ficam registrados **juntos** na Resolution do bug.

O vínculo reverso vive também do lado da spec: `_solucao_sdd/traceability/bugs.md` é um espelho gerado listando, por seção de spec, os bugs que a atingem.

---

## Non-destructive

Os Bug Agents escrevem apenas dentro de `_solucao_bugs/`, além dos adendos em `_solucao_sdd/addenda/` e do espelho gerado em `_solucao_sdd/traceability/`. O código do projeto só muda pelos dois gates de aprovação, com diffs explícitos. Bugs com `visibility: restricted` (segurança) ficam fora das views públicas e nunca chegam a harness externos.
