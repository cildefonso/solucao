...
- **Fase 2:** genera `target_screens.md` (con YAML embebido por pantalla) y `screen_deviation_log.md`. Cuando el oráculo legado corre, emite golden files en `_solucao_sdd/screens/golden/` más un `manifest.yaml` que el Inspector consume para tests de paridad visual.

En proyectos sin UI (batch, API puro, daemons) emite `mode: skipped` y el Inspector salta la paridad visual.

**Produce:** `screen_modernization_decision.md`, `target_screens.md`, `screen_deviation_log.md`, y opcionalmente `_solucao_sdd/screens/golden/*` + `manifest.yaml`.
