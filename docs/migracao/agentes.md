...
The agents run in fixed sequence. Each one reads what the previous produced and adds its own artifact. `/solucao-migrate` orchestrates everything.
...
**Command:** `/solucao-paradigm-advisor` (usually invoked by `/solucao-migrate`)
...
**Command:** `/solucao-screen-translator` (usually invoked by `/solucao-migrate`, between Designer and Inspector)
...
- **Phase 2:** generates `target_screens.md` (with YAML embedded per screen) and `screen_deviation_log.md`. When the legacy oracle runs, also emits golden files under `_solucao_sdd/screens/golden/` plus a `manifest.yaml` the Inspector consumes for visual parity tests.

In projects without UI (batch jobs, pure APIs, daemons) emits `mode: skipped` and the Inspector skips visual parity.

**Produces:** `screen_modernization_decision.md`, `target_screens.md`, `screen_deviation_log.md`, optional `_solucao_sdd/screens/golden/*` + `manifest.yaml`.

---

## 6. Inspector

**Command:** `/solucao-inspector`
...
You almost never need to call an isolated agent. `/solucao-migrate` orchestrates everything. But if an agent failed or you want to rerun from a specific point:

```
/solucao-migrate --resume                    # resumes from the last completed agent
/solucao-migrate --regenerate=designer       # deletes Designer + Inspector outputs and redoes them
```
