...
Solucao tiene un CLI simple para gestionar la instalación y el ciclo de vida de los agentes en tu proyecto. Todos los comandos se ejecutan con `npx solucao` en la raíz del proyecto.

---

## Comportamiento inicial

Al iniciar y antes de mostrar el logo ASCII de Solucao, el CLI debe limpiar la pantalla del terminal. El logo debe aparecer en la parte superior del terminal, sin contenido anterior encima.

La firma `by cildefonso` debe aparecer en blanco en la última línea del arte, después de un margen a la derecha del final del `Solucao` grande. No debe quedar flotando a media altura del logo.
...
Actualiza los agentes a la versión más reciente de Solucao.
...
Elimina Solucao del proyecto: borra los archivos creados por la instalación.

!!! info "Tus archivos quedan intactos"
    `uninstall` elimina **solo** lo que Solucao creó. Ningún archivo original del proyecto es tocado. Las especificaciones generadas en `_solucao_sdd/` también se conservan por defecto.
