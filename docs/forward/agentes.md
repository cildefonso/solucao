...
Ten agents make up the Code Forward Agents Team. The orchestrator (`/solucao-forward`) detects the physical stage of the active feature and suggests the next skill. The other nine cover the lifecycle from a free-form idea to running code.
...
There is a `CONTINUAR` checkpoint between agents. Each skill verifies its own preconditions and refuses to run if a required predecessor is missing. `solucao-coding` is the strictest: it aborts unless `_solucao_sdd/` holds a context anchor, either the legacy pair `architecture.md` + `domain.md` (from `/solucao`) or the greenfield pair `prd.md` + at least one spec in `sdd/` (from `/solucao-new`), to keep the specs-to-code bridge solid.

---

## 1. Solucao Forward (orchestrator)

**Command:** `/solucao-forward`
...
**Command:** `/solucao-requirements`

Turns a free-form idea ("I want users to export their invoices as PDF") into a complete `requirements.md`, anchored to `_solucao_sdd/architecture.md`, `domain.md`, `state-machines.md` and the glossary. Marks open points with `[DOUBT]`, lists gaps and registers the feature in `.solucao/active-requirements.json`.
...
**Produces:** `.solucao/principles.md` and `principles-impact-YYYYMMDD.md` on each change.

---

## 10. Resume

**Command:** `/solucao-resume`
...
`/solucao-forward` is the recommended entry point when you do not remember where the active feature stopped. But each skill can be activated standalone:

```
/solucao-forward                 # detect stage and suggest next skill
/solucao-requirements <idea>     # new feature from a free-form idea
/solucao-clarify                 # resolve [DOUBT] markers in requirements.md
/solucao-quality                 # audit writing clarity (read-only)
/solucao-plan                    # delta over legacy from requirements.md
/solucao-to-do                   # atomic actions from roadmap.md
/solucao-audit                   # cross-check between the three docs (read-only)
/solucao-coding                  # execute actions.md
/solucao-principles              # manage durable rules
/solucao-resume                  # swap with a paused feature
```
