...
Every artifact this Team produces is anchored to the Discovery output (`_solucao_sdd/`). When code finally lands, two trails are left behind for future Solucao runs: `legacy-impact.md` (what was changed in the legacy) and `regression-watch.md` (what must remain true on the next extraction).
...
`/solucao-coding` requires a context anchor in `_solucao_sdd/`: either the legacy extraction (`architecture.md` + `domain.md`, from `/solucao`) or the greenfield artifacts (`prd.md` + at least one spec in `sdd/`, from `/solucao-new`). Without any anchor, it refuses to run: the code bridge would be hollow and Forward would degrade into a generic scaffolding tool.
...
```
/solucao-forward         (orchestrator, detects current stage and suggests the next skill)
        │
        ▼
/solucao-requirements
        │     idea  →  requirements.md (with [DOUBT] markers, gaps, glossary)
        ▼
/solucao-clarify           (optional)
        │     up to 5 targeted questions  →  resolves [DOUBT] in place
        ▼
/solucao-quality         (optional, read-only)
...
/solucao-to-do
        ...
        ▼
/solucao-audit           (optional, read-only)
...
`/solucao-forward` is the optional entry point for the cycle: it looks at the current state and tells you which skill to run next. Handy when you do not remember where you stopped.
`/solucao-principles` and `/solucao-resume` run outside this linear flow. The first manages durable project rules; the second swaps the active feature with one paused on the side.

---

## Feature lifecycle

A feature is identified by `<NNN>-<short-name>` (e.g. `012-pdf-export`). The active one is tracked in `.solucao/active-requirements.json`; previously active ones can sit in a `paused-features` queue and be resumed at any time.

```
.solucao/active-requirements.json
{
  "feature-dir": "_solucao_forward/012-pdf-export",
  "feature-id": "012",
  "short-name": "pdf-export",
  "started-at": "2026-05-08T14:22:00Z",
  "paused-features": [
    { "feature-dir": "_solucao_forward/011-multi-tenant", "paused-from-stage": "plan", ... }
  ]
}
```
...
|-------|-------|--------|------------|
| `solucao-forward` | orchestrator | (none, routing only) | `state.json`, `active-requirements.json`, feature artifacts in `_solucao_forward/` |
| `solucao-requirements` | requirements | `requirements.md`, `active-requirements.json` |  |
| `solucao-clarify` | clarify | `requirements.md` (in-place edits) |  |
| `solucao-quality` | quality | `audit/requirements-audit.md` | `requirements.md` |
| `solucao-plan` | plan | `roadmap.md`, `investigation.md`, `data-delta.md`, `onboarding.md`, `interfaces/*` |  |
| `solucao-to-do` | to-do | `actions.md` |  |
| `solucao-audit` | audit | `audit/cross-check.md` | requirements + roadmap + actions |
| `solucao-coding` | coding | source code, `actions.md` checkboxes, `progress.jsonl`, `legacy-impact.md`, `regression-watch.md` |  |
| `solucao-principles` | principles | `.solucao/principles.md`, `principles-impact-YYYYMMDD.md` |  |
| `solucao-resume` | resume | `active-requirements.json` (swap), no feature artifacts |  |

### `solucao-requirements`
Turns a free-form idea into a complete `requirements.md`, anchored to the legacy via `_solucao_sdd/` (architecture, domain, state machines, glossary). It detects when a previous feature is already in progress and asks the user whether to continue, run in parallel (pausing the previous one), or abandon, never decides on its own.
...
│
└── _solucao_forward/                (forward_folder from .solucao/state.json)
    └── <NNN>-<short-name>/          (one per feature)
        ...
        ├── regression-watch.md
        └── audit/
            ├── requirements-audit.md   (solucao-quality)
            └── cross-check.md          (solucao-audit)
```

The Code Forward Agents never touch the legacy code unsupervised; that only happens inside `/solucao-coding`, and even then the skill leaves the two trails above so the next Discovery run can detect any drift.

---

## Hooks

Each skill applies the matching hook entries from `.solucao/hooks.yml`, in two slots: `before-<stage>` and `after-<stage>` (e.g. `before-plan`, `after-coding`). Hooks marked `optional: true` show up as suggested commands; `optional: false` blocks the skill until executed. The `condition` field is recorded but never auto-evaluated, the user owns that decision.
