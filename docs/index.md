...
You know that system nobody wants to touch? The one that's been running for 10 years, makes money every day, but nobody really knows what it does on the inside? Solucao was built for it.

---

## What is Solucao?

Solucao is a specification reverse-engineering framework. You install it inside a legacy project, activate an AI agent you already use, and it coordinates a team of specialists to analyze the code and generate complete, traceable, ready-to-use specifications for any coding agent.

**In other words:** Solucao turns undocumented code into operational contracts that an AI agent can understand and use to safely evolve the system.
...
- **Why Solucao exists**
    ...
    [:octicons-arrow-right-24: Read more](por-que-solucao.md)
...
!!! warning "Solucao never touches your files"
    Agents write **only** to `.solucao/` and `_solucao_sdd/`. No file in your project is modified, deleted, or overwritten. Ever.

!!! info "No API keys"
    Solucao does not request, store, or transmit API keys from any service. The intelligence comes from the agent you already use in your environment, like Claude Code, Codex, Gemini CLI etc.
