...
2. Asks which agent **Teams** to install. `Solucao Agents Core` is always included; `Migration Agents`, `Code Forward Agents` and `Pricing and Size Agents` come pre-checked; `Translators N8N->Specs->Python` is unchecked by default. The CLI expands each chosen Team into its agents
...
6. Creates the `.solucao/` structure with state, configuration, and plan
...
```
legacy-project/
├── .solucao/               ← analysis state, config, and context
...
└── _solucao_sdd/           ← where specs will be generated (empty initially)
```
...
!!! warning "Strong recommendation: make a backup"
    Although Solucao never modifies your files, AI agents can make mistakes. Before starting the analysis:
