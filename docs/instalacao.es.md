...
2. Pregunta qué **Teams** de agentes instalar. `Solucao Agents Core` siempre está incluido; `Migration Agents`, `Code Forward Agents` y `Pricing and Size Agents` vienen marcados; `Translators N8N->Specs->Python` queda desmarcado por defecto. El CLI expande cada Team elegido en sus agentes
...
```
proyecto-heredado/
├── .solucao/               ← estado, config y contexto del análisis
...
└── _solucao_sdd/           ← donde se generarán las specs (vacío inicialmente)
```
...
!!! warning "Recomendación fuerte: haz un backup"
    Aunque Solucao nunca modifica tus archivos, los agentes de IA pueden cometer errores. Antes de iniciar el análisis:
