...
Compare `.solucao/version` com o npm registry. Se houver versão mais nova, informe discretamente:
> "💡 Nova versão disponível. Execute `npx solucao update` quando quiser atualizar."
...
Salve as respostas em `.solucao/state.json` nos campos `user_name`, `chat_language`, `doc_language` e `project`.
...
>
> Trabalharei em etapas, salvando o progresso a cada fase. Se a sessão for interrompida, basta digitar `solucao` novamente para continuar de onde paramos."
...
Após aprovação do plano, atualize `.solucao/state.json`:
