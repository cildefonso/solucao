...
1. Salve checkpoint em `.solucao/state.json` imediatamente
2. Diga: "[Nome], vou pausar aqui. Tudo está salvo. Digite `/solucao` em uma nova sessão para continuar."
...
> 2. Pausar aqui, digitar `/clear` para limpar o contexto, e voltar com `/solucao` em sessão nova (recomendado se a sessão atual já está longa)
>
> Pressione 1, 2, ou apenas digite CONTINUAR para opção 1."

Antes de oferecer a opção 2, **confirme que o checkpoint está salvo** em `.solucao/state.json` (campo `phase`, `completed`, `checkpoints` do agente que acabou de rodar). Sem checkpoint válido, oferecer pausa é arriscado.
