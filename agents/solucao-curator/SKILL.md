...
1. As specs do legado em `_solucao_sdd/`.
2. O critério registrado em `migration_brief.md`.
3. O paradigma escolhido em `paradigm_decision.md`.

## Pré-requisitos

- `_solucao_sdd/migration/migration_brief.md` existe.
- `_solucao_sdd/migration/paradigm_decision.md` existe (Paradigm Advisor já rodou).

Se algum faltar, pare e instrua o usuário a executar `/solucao-migrate` ou rodar o agente faltante.

## Inputs

- `_solucao_sdd/migration/migration_brief.md`
- `_solucao_sdd/migration/paradigm_decision.md`
- `_solucao_sdd/<unit>/requirements.md` e `_solucao_sdd/<unit>/design.md` de cada unit (specs por unit, contêm regras de negócio)
...
- `_solucao_sdd/migration/target_business_rules.md`
- `_solucao_sdd/migration/discard_log.md`
- Atualização de `_solucao_sdd/migration/ambiguity_log.md` (criar se não existir)
...
Leia o `paradigm_decision.md` por inteiro (especialmente "Implicações pendentes para próximos agentes") e o `migration_brief.md`. Em seguida, leia, em cada pasta de unit dentro de `_solucao_sdd/`, os arquivos `requirements.md` e `design.md`, mais os artefatos auxiliares.
...
- Não modificar artefatos do `_solucao_sdd/` fora da pasta `migration/`.
