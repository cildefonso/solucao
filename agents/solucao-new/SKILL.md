...
```
/solucao-new (você está aqui)
       │
       ▼ chama
   solucao-ideator            → ideation.md
       │
       ▼ chama (guiado: após CONTINUAR | expresso: direto)
   solucao-researcher         → personas.md
       │
       ▼ chama (guiado: após CONTINUAR | expresso: direto)
   solucao-drafter            → prd.md
       │
       ▼ chama (guiado: após CONTINUAR | expresso: direto)
   solucao-spec-sdd           → sdd/<componente>.md
       │
       ├── modo guiado: handoff, sugere /solucao-forward
       │
       ▼ modo expresso: continua direto
   solucao-requirements       → <forward_folder>/<NNN>-<short>/requirements.md
       │
       ▼ (clarify pulado, [DÚVIDA] vira premissa 🟡)
   solucao-plan               → roadmap.md, investigation.md, ...
       │
       ▼
   solucao-to-do              → actions.md
       │
       ▼
   solucao-coding             → código + progress.jsonl + legacy-impact.md + regression-watch.md
```
...
3. Garanta que `_solucao_sdd/` existe (criação recursiva, sem `.gitkeep`). Mesmo padrão do `/solucao-forward`.
...
2. Se `stage` for um valor do pipeline (`ideator`, `researcher`, `drafter`, `spec-sdd`, `forward-requirements`, `forward-plan`, `forward-todo`, `forward-coding`), apresente menu:
   ...
   ```
   Já existe um pipeline /solucao-new em andamento:
     - Estágio atual: <stage>
...
- `ideator` → próximo é `solucao-researcher`
- `researcher` → próximo é `solucao-drafter`
- `drafter` → próximo é `solucao-spec-sdd`
- `spec-sdd` → modo guiado: handoff final (pipeline completo); modo expresso: próximo é `solucao-requirements`
- `forward-requirements` → próximo é `solucao-plan` (só existe em modo expresso)
- `forward-plan` → próximo é `solucao-to-do`
- `forward-todo` → próximo é `solucao-coding`
- `forward-coding` → retome as ações `[ ]` pendentes de `actions.md` via `solucao-coding`; se todas `[X]`, exiba o relatório final expresso
...
A partir de qual agente?
  [1] solucao-ideator (refaz brainstorm)
  [2] solucao-researcher (refaz personas)
  [3] solucao-drafter (refaz PRD)
...
- **Guiado:** um agente por vez, com CONTINUAR entre eles. Termina nas specs SDD com handoff para `/solucao-forward`.
...
3. Senão, pergunte usando o menu interativo da engine (no Claude Code, `AskUserQuestion`; em engines sem suporte, menu numerado):
   ...
   > Como você quer executar o `/solucao-new`?
   >
   > 1. **Guiado** (padrão): passo a passo, você aprova cada etapa. Termina nas specs SDD, prontas para o `/solucao-forward`.
...
Se o usuário passou argumento livre ao `/solucao-new`, use como brief inicial. Senão, pergunte:
...
---
Gerado por /solucao-new em <ISO 8601>
```
...
|---|---|---|---|
| 1 | solucao-ideator | `_solucao_sdd/ideation.md` | `researcher` |
| 2 | solucao-researcher | `_solucao_sdd/personas.md` | `drafter` |
| 3 | solucao-drafter | `_solucao_sdd/prd.md` | `spec-sdd` |
...
> `<user_name>`, o pipeline `/solucao-new` terminou. Artefatos gerados em `_solucao_sdd/`:
>
...
>
> Todos os artefatos têm selo 🟡 (planejado). Próximo passo: rodar `/solucao-forward`, que vai consumir esses artefatos e iniciar o ciclo de evolução até o código.
>
> Digite **CONTINUAR** para iniciar `/solucao-forward`, ou pause aqui.

Se a engine permitir, ative `/solucao-forward` quando o usuário responder CONTINUAR. Senão, apenas oriente.

## Modo expresso

O modo expresso executa os mesmos agentes do modo guiado e, ao final das specs, emenda automaticamente no ciclo forward até o código. Todas as decisões são coletadas em uma **entrevista única no início**, no mesmo padrão do `/solucao-autonomous`. Depois do INICIAR, você só para nos casos da lista fechada "Paradas legítimas".
...
2. **solucao-ideator:** não entrevista. Sintetiza `ideation.md` diretamente das respostas do bloco de ideação da entrevista.
3. **solucao-researcher:** não pergunta. Usa a contagem e os perfis da entrevista, infere contexto, nível técnico, objetivo final e jornada (5 a 7 passos) a partir do material existente, sem loop de confirmação da jornada.
4. **solucao-drafter:** pula as perguntas de cobertura, usa o bloco 5 da entrevista. Gaps viram `[INDEFINIDO]`.
...
Ao concluir o `solucao-spec-sdd`, NÃO pare no handoff. Atualize `stage` para `forward-requirements` e continue:

1. **solucao-requirements** com argumento derivado da seção "Escopo (in)" do `prd.md`: a primeira feature é o MVP descrito no PRD. Overrides:
...
2. **solucao-clarify é pulado.** `[DÚVIDA]` remanescentes viram premissas 🟡 no `roadmap.md`, comportamento que o `solucao-plan` já prevê. A pergunta "prefere rodar clarify antes?" é respondida pelo orquestrador: prosseguir.
3. **solucao-plan** e **solucao-to-do** com o mesmo contexto greenfield (specs SDD e PRD no lugar dos artefatos de descoberta).
4. **solucao-coding** em cenário greenfield, que o próprio skill já suporta nativamente: a âncora é `<output_folder>/prd.md` mais pelo menos uma spec em `<output_folder>/sdd/` (no lugar de `architecture.md` + `domain.md`), e `legacy-impact.md`/`regression-watch.md` se adaptam conforme descrito no SKILL.md do coding. Reforço do modo expresso:
...
3. **Ação do `solucao-coding` falhou:** a fase para e o problema é relatado, comportamento herdado do coding.
...
5. **Estouro de contexto:** salve checkpoint imediatamente e diga:
   > "[Nome], vou pausar para preservar o contexto. Tudo salvo. Digite `/solucao-new` em uma nova sessão para retomar de onde paramos."
...
6. Próximos passos: rodar `/solucao` para extrair specs 🟢 do código recém-criado e fechar o ciclo, ou `/solucao-docs` para documentação viva.
...
2. Diga: "`<user_name>`, vou pausar aqui. O estado está salvo. Digite `/solucao-new` em uma nova sessão para retomar de onde paramos."
