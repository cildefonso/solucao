---
name: solucao-debugger-graph
description: Gerador de views do Time Solucao Bugs. Varre os bug.md (source of truth), valida invariantes e regenera as projeções: índice por status/phase, catálogo compacto, matriz esparsa de relações BUG↔BUG, grafo mermaid com clusters e impact score, e a matriz de rastreabilidade BUG↔SPEC nas duas pontas (incluindo o espelho em `_solucao_sdd/traceability/bugs.md`). Use quando o usuário digitar "/solucao-debugger-graph", "solucao-debugger-graph", "panorama dos bugs", "grafo de bugs", "regenerar índice de bugs" ou pedir a matriz de rastreabilidade de bugs.
license: MIT
...
1. Leia `.solucao/state.json` (`output_folder`, `doc_language`)
2. Os bugs vivem agrupados por contexto: `_solucao_bugs/<contexto>/bugs/`. Se nenhuma pasta de contexto existir, informe que não há bugs registrados e aponte `/solucao-debugger`
3. Escopo: com argumento (`/solucao-debugger-graph carrinho-de-compras`), regenere só aquele contexto; sem argumento, regenere TODOS os contextos encontrados, um a um

## Etapa 1: varredura e validação

1. Leia o front matter de TODOS os `_solucao_bugs/*/bugs/*/bug.md` (só o front matter; corpo apenas quando precisar de detalhe). A validação de invariantes é sempre GLOBAL, cruzando contextos (ID duplicado entre contextos é erro)
...
Cada contexto tem as próprias views em `_solucao_bugs/<contexto>/generated/`, cobrindo só os bugs daquele contexto (relações que cruzam contextos aparecem com o contexto do alvo indicado). Todas com cabeçalho `<!-- GENERATED, DO NOT EDIT: regenerado por /solucao-debugger-graph em <ISO 8601> a partir de N bugs -->`. Escrita atômica. Crie `generated/` do contexto se ainda não existir.
...
1. Linha de metadados: gerado por /solucao-debugger-graph, data, N bugs, N inconsistências, legenda "arestas tracejadas = relação proposed (hipótese)"
...
Gere `_solucao_sdd/traceability/bugs.md` (crie a pasta `traceability/` se não existir):
...
4. Nenhum outro arquivo de `_solucao_sdd/` é tocado. O espelho registra o vínculo; mudança de conteúdo de spec é assunto dos adendos.
...
> Digite **CONTINUAR** para corrigir o bug de maior impacto com `/solucao-debugger-fix <ID>`, registrar um novo com `/solucao-debugger`, ou encerrar.

## Regra absoluta

**Nunca apague, modifique ou sobrescreva arquivos pré-existentes do projeto.**
Este skill escreve APENAS em `_solucao_bugs/<contexto>/generated/` e `_solucao_sdd/traceability/bugs.md` (ambos views regeneráveis). Os `bug.md` são somente leitura aqui: inconsistência se reporta, não se conserta.
