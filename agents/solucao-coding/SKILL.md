---
name: solucao-coding
description: Conduz a execução do actions.md em código. Atualiza checkboxes para [X], escreve progress.jsonl, gera legacy-impact.md e regression-watch.md. Funciona ancorado no legado (extração do Discovery em `_solucao_sdd/`) ou em greenfield (prd.md + specs SDD do `/solucao-new`). Use quando o usuário digitar "/solucao-coding", "solucao-coding", "executar plano" ou pedir para começar a codar a feature ativa. Último skill do ciclo forward, depois de `/solucao-to-do` (e opcionalmente `/solucao-audit` ou `/solucao-quality`).
license: MIT
...
2. **Greenfield:** `_solucao_sdd/` contém `prd.md` E pelo menos uma spec em `_solucao_sdd/sdd/` (artefatos do `/solucao-new`). Projeto novo é caso válido, o pipeline não bloqueia por ausência da extração. Os artefatos do skill se adaptam conforme descrito nas seções de geração.

Se existirem as duas âncoras (projeto que rodou `/solucao` e `/solucao-new`), use a de legado como principal e as specs SDD como complemento.
...
1. Leia `.solucao/active-requirements.json`
   1.1. Se ausente, aborte com mensagem apontando `/solucao-requirements`
2. Verifique a existência de `feature-dir/actions.md`
   2.1. Se ausente, aborte com mensagem apontando `/solucao-to-do`
3. Verifique a âncora de contexto:
   ...
   3.2. **Âncora greenfield:** `_solucao_sdd/` existe E contém `prd.md` E pelo menos um arquivo `.md` em `_solucao_sdd/sdd/`. Se satisfeita (e a de legado não), registre o cenário como **greenfield**, informe ao usuário ("Sem extração de legado, vou ancorar nos artefatos do `/solucao-new`: `prd.md` e specs SDD.") e siga para o passo 4.
   3.3. Se NENHUMA das duas âncoras estiver satisfeita, aborte com a mensagem:

       > 🛑 `/solucao-coding` exige uma âncora de contexto em `_solucao_sdd/` e não encontrei nenhuma:
       >
       > - **Legado:** `architecture.md` + `domain.md` (gere com `/solucao`)
       > - **Greenfield:** `prd.md` + specs em `sdd/` (gere com `/solucao-new`)
       >
...
3. Atribua severidade alinhada com `/solucao-audit` (CRITICAL, HIGH, MEDIUM, LOW)
...
3. Seção "Histórico de re-extrações" inicialmente vazia, será preenchida pelo agente reverso quando rodar `/solucao` de novo
...
5. Aviso explícito: rode `/solucao-sync` para converger a entrega em `_solucao_sdd/addenda/` e mantenha no radar rodar `/solucao` (re-extração) novamente em algum momento futuro para fechar o ciclo
...
> Digite **CONTINUAR** para prosseguir com `/solucao-sync` (convergência da entrega na extração) ou outra ação que o usuário quiser.
