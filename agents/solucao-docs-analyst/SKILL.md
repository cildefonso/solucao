...
Segundo agente do pipeline `/solucao-docs`. Reusa os JSONs intermediários do Mapper (`modules.json`, `deps.json`). Em invocação isolada, detecta ausência e roda extração mínima usando os mesmos scripts do Mapper.

## Inputs

- `_solucao_docs/assets/data/modules.json` (do Mapper, ou extrai sob demanda)
...
- `_solucao_docs/metricas.html` (dashboard 4+ gráficos)
- `_solucao_docs/timeline.html` (omitida se chronicle ausente)
- `_solucao_docs/assets/data/metrics.json`
- `_solucao_docs/assets/data/timeline.json` (apenas se chronicle existir)
...
3. Verifique presença de `modules.json` e `deps.json`. Se ausentes, invoque os scripts do Mapper para gerá-los (`extract_modules.py`, `extract_deps.py`). Política de cache em `agents/solucao-docs-mapper/references/extraction-policy.md`.
4. Verifique se `_solucao_docs/assets/vendor/highcharts.js` (e módulos associados) existe. Se ausente em modo isolado, execute o Passo 0 do Publisher (`agents/solucao-docs-publisher/SKILL.md`) lendo `vendor-pins.yaml` para baixar Highcharts + módulos com retry de CDN. No modo orquestrado, isso já foi feito na Fase 0.
...
Salve em `_solucao_docs/assets/data/metrics.json`.
...
3. Adapte ao chassis `viewer.html`:
   - Preencha marcadores padrão (TITLE = "Métricas", PAGE_ID = "metricas", SOLUCAO_CATEGORY = "diagram", SOLUCAO_PRODUCER_AGENT = "solucao-docs-analyst", SOLUCAO_TEMPLATE = "metricas", VISUAL_STYLE, GENERATED_AT). Deixe `<!-- NAV_LINKS -->` como está (Publisher backpatcha).
...
5. Salve em `_solucao_docs/metricas.html`.
...
7. Salve em `_solucao_docs/timeline.html`.
...
`_solucao_docs/.backup-<YYYYMMDD-HHMMSS>/` antes de sobrescrever.

## Diretiva non-destructive

Apenas escreve em `_solucao_docs/`. `chronicle.md`, `modules.json`, `deps.json` são lidos sem modificação.
...
> Tempo: [N]s
>
> [Se invocado isolado:] Próximo natural: `/solucao-docs-storyteller`, ou `/solucao-docs-publisher` para reintegrar o index.
>
