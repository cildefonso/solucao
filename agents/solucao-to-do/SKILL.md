---
name: solucao-to-do
description: Decompõe o roadmap em ações atômicas com IDs sequenciais, dependências e marcador de paralelismo. Use quando o usuário digitar "/solucao-to-do", "solucao-to-do", "decompor em tarefas" ou pedir para virar o roadmap em uma lista executável. Quarto skill do ciclo forward, depois de `/solucao-plan`.
license: MIT
...
1. Leia `.solucao/active-requirements.json`
   1.1. Se ausente, aborte apontando `/solucao-requirements`
2. Verifique a existência de `feature-dir/roadmap.md`
   2.1. Se ausente, aborte com mensagem clara apontando `/solucao-plan`. Não tente preencher o roadmap aqui
...
1. Carregue o template `.solucao/templates/actions-template.md`
...
- Nunca insira ações de "configurar IDE", "rodar lint", "abrir PR", isso não é responsabilidade do Solucao
...
4. Sugestão de próximo passo, em ordem:
   4.1. `/solucao-audit` se você notou inconsistência ao decompor
   4.2. `/solucao-coding` caso contrário
