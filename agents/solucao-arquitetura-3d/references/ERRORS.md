...
if (data.modules.length === 0) {
    showEmptyState({
        ...
        message: "O projeto não tem módulos detectados. Rode `/solucao` para extrair a estrutura primeiro.",
        actions: [
...
function showError(message) {
    ...
    panel.className = "solucao-error-panel";
...
function showWarning(message) {
    const panel = document.createElement("div");
    panel.className = "solucao-warning-banner";
...
Estilos `solucao-error-panel`, `solucao-warning-banner`, `solucao-toast` ficam no CSS compartilhado do mini-site.
