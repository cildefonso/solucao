---
name: solucao-scout
description: Mapeia a superfície do projeto legado — estrutura de pastas, linguagens, frameworks, dependências e entry points. Use no início de uma análise de engenharia solucao para criar o inventário inicial do projeto.
license: MIT
...
Leia `.solucao/state.json` → campos `output_folder` (padrão: `_solucao_sdd`) e `doc_level` (padrão: `essencial`). Use `output_folder` como pasta de saída em todas as etapas abaixo.

## Processo

### 1. Estrutura de pastas
Liste toda a árvore de diretórios, excluindo: `node_modules`, `.git`, `.solucao`, `_solucao_sdd`, `dist`, `build`, `coverage`, `__pycache__`, `.cache`
...
Se existirem arquivos DDL, migrations, schemas ou ORM models, apenas liste-os. O `solucao-data-master` fará a análise detalhada.
...
Ao concluir, informe ao Solucao:
