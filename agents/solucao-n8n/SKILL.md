...
### Pasta de saída: `_solucao_n8n/<slug>/`
...
5. Se a pasta `_solucao_n8n/<slug>/` já existir, pergunte: sobrescrever, criar versão nova (`-v2`, `-v3`...) ou cancelar.
...
Se `.solucao/state.json` ainda não existir, crie a partir do template em `templates/state.json` e popule:

- `version`: ler de `package.json` do Solucao (campo `version`)
...
- `output_folder`: `_solucao_sdd` (default do pipeline principal)
- `phase`: `null` (deixar o `/solucao` definir como `reconhecimento` ao iniciar)
- `engines`: lista vazia (será preenchida pelo /solucao)
- `agents`: lista vazia
- `created_files`: lista vazia
- Adicione um campo `source` com valor `"n8n"` e `source_artifacts` apontando para `_solucao_n8n/<slug>/` para que o Scout saiba que existe pré-análise.
...
> A análise foi iniciada a partir de um workflow N8N. A pré-análise gerou specs em `_solucao_n8n/<slug>/`. O Scout deve incluir esses artefatos no inventário.
...
Os artefatos da spec ficam em `_solucao_n8n/<slug>/`. Os arquivos de estado para o pipeline principal ficam em `.solucao/`. Os JSONs de entrada permanecem em `n8n_json_workflows/` intactos. Não escrever em `_solucao_sdd/` aqui (essa pasta é populada pelos agentes do pipeline principal a partir do `/solucao`).
...
Sugira ao usuário:
1. Revisar a spec em `_solucao_n8n/<slug>/`
2. Digitar `/solucao` para acionar o pipeline completo (Scout em diante) sobre a pré-análise N8N
3. Ou processar outro workflow direto, se houver mais arquivos em `n8n_json_workflows/`

Termine com: `Digite CONTINUAR para processar outro workflow, ou /solucao para iniciar o pipeline principal.`

## Regras absolutas

- Nunca modificar o arquivo JSON original em `n8n_json_workflows/`
- Escrever apenas em `n8n_json_workflows/` (criar a pasta), `_solucao_n8n/` e `.solucao/`
- Nunca sobrescrever `.solucao/state.json` se já existir, apenas atualizar os campos `source` e `source_artifacts`
