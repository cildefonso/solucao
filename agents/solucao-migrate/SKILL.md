---
name: solucao-migrate
description: "Orquestrador do Time de Migração do Solucao. Conduz o pipeline de migração após o `/solucao` ter populado o _solucao_sdd/. Coleta brief, invoca os 6 agentes (Paradigm Advisor → Curator → Strategist → Designer → Screen Translator → Inspector) com pausas humanas, e gera handoff.md final. Use quando o usuário digitar `/solucao-migrate`, `solucao-migrate`, `migrar sistema`, `iniciar migração`."
license: MIT
...
1. Verifique que `_solucao_sdd/` existe.
   - Se não: encerre com a mensagem:
     > "Não encontrei `_solucao_sdd/`. Execute `/solucao` primeiro para gerar as specs do sistema legado."
2. Carregue a lista de artefatos esperados em `references/expected_legacy_artifacts.yaml` (cópia local da skill).
3. Para cada artefato `required: true`, verifique presença em `_solucao_sdd/` (considere também aliases declarados).
   - Se algum faltar: liste todos os faltantes, informe que o pipeline está bloqueado, peça ao usuário rodar `/solucao` novamente, e encerre.

### Passo 2: Estado e modo

1. Se `_solucao_sdd/migration/.state.json` **não existir**: este é primeiro run; siga para o passo 3.
...
Se `_solucao_sdd/migration/migration_brief.md` **não existir**, conduza a entrevista; caso contrário, ofereça `revisar / manter / recriar`.
...
Renderize `_solucao_sdd/migration/migration_brief.md` usando o template em `references/templates/migration_brief.md`.

### Passo 4: Inicializar `.state.json`

Crie `_solucao_sdd/migration/.state.json` a partir do template `references/state.json`. Preencha `startedAt`, `engine`, `solucaoVersion`. Marque `currentAgent.agent = "paradigm_advisor"`, `currentAgent.phase = null`, `currentAgent.status = "running"`, `currentAgent.topologyApproved = false`.
...
2. Ative a skill do agente (`solucao-paradigm-advisor`, `solucao-curator`, `solucao-strategist`, `solucao-designer`, `solucao-screen-translator`, `solucao-inspector`). Se a engine não suportar ativação direta por nome, instrua a leitura de `.agents/skills/<id>/SKILL.md` no contexto atual.
...
- **Engines sem TTY interativo**: escreva `_solucao_sdd/migration/pending_decisions.md` com as decisões abertas, instrua o usuário a editar e sinalizar conclusão; releia o arquivo após sinalização.
...
Após cada agente, integre itens ⚠️ e pendências em `_solucao_sdd/migration/ambiguity_log.md`. Ao final, organize em três grupos:
...
1. Renderize `_solucao_sdd/migration/handoff.md` usando o template em `references/templates/handoff.md`.
...
> - Tempo total: <minutos>
>
> Próximo passo: abra `_solucao_sdd/migration/handoff.md` no agente de codificação que vai implementar o sistema novo."

Grave log completo em `_solucao_sdd/migration/.logs/<timestamp>-migrate.log` com timestamp por entrada e identificação do agente. Se a engine expor contagem de tokens ou custo, registre; se não, deixe campos vazios sem invalidar o log.
...
1. Confirme com o usuário (operação destrutiva no escopo de `_solucao_sdd/migration/` e `_solucao_sdd/screens/`).
2. Faça backup em `_solucao_sdd/migration/.backup-<timestamp>/` e, se aplicável ao Screen Translator, em `_solucao_sdd/screens/.backup-<timestamp>/`.
3. Apague artefatos:
...
- **`_solucao_sdd/` incompleto**: lista artefatos faltantes e aborta.
...
- **Não modificar nada fora de `_solucao_sdd/migration/`.**
- Artefatos pré-existentes em `_solucao_sdd/` são **lidos**, nunca modificados.
