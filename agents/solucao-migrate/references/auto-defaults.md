...
Quando o usuário invoca `/solucao-migrate --auto`, o orquestrador pula pausas humanas e aplica estes defaults. Antes de iniciar, o aviso ao usuário lista cada um deles. Cada item auto-aplicado é registrado em `ambiguity_log.md` com tag `auto-decidido` para revisão posterior.
...
- **Pré-requisitos Discovery ausentes** (`_solucao_sdd/design-system/` ou `_solucao_sdd/ui/inventory.md`): cria `tokens-derived.md` mínimo e constrói inventário só a partir do código fonte; alerta no `ambiguity_log.md`.
