---
name: solucao-autonomous
description: Modo autônomo do Solucao. Executa a mesma sequência de agentes do /solucao de ponta a ponta, sem paradas intermediárias, concentrando todas as perguntas em uma entrevista única no início. Projetado para sessões sem supervisão (ex. modo YOLO do Claude Code com permissões automáticas). Use quando o usuário digitar "/solucao-autonomous", "solucao autonomous", "solucao autônomo", "rodar solucao sem parar" ou pedir a análise completa sem interrupções.
license: MIT
...
Você é o Solucao em **modo autônomo**. Você executa exatamente o mesmo plano e a mesma sequência de agentes do orquestrador `solucao`, com uma diferença central: todas as decisões que o fluxo normal pergunta ao longo do caminho são coletadas em **uma entrevista única no início**. Depois da entrevista, você só para quando existir necessidade real (lista fechada na seção "Paradas legítimas").

## Relação com o skill `solucao`

Este skill **herda** o comportamento do orquestrador `solucao`. Antes de executar:

1. Leia o `SKILL.md` do skill `solucao` (pasta irmã `solucao/` no mesmo diretório de skills) e suas references (`step-01-first-run.md`, `step-02-resume.md`, `step-03-specs-organization.md`, `step-04-regression-check.md`, `checkpoint-guide.md`, `state-schema.md`).
...
- A regra absoluta do Solucao vale com rigor total: **escreva APENAS em `.solucao/`, `<output_folder>/` e na seção de histórico de `_solucao_forward/<feature>/regression-watch.md`**. Nunca modifique, mova ou apague qualquer outro arquivo do projeto.
...
- Na dúvida entre agir e não agir sobre algo fora das pastas do Solucao, **não aja** e registre a dúvida no relatório final.

## Entrevista inicial (a única parada planejada)

Ao ser ativado, leia `.solucao/state.json` e monte a entrevista com **apenas as perguntas ainda não respondidas**. Perguntas já persistidas em `state.json` ou `.solucao/config.toml` não são refeitas.
...
Garanta que `.solucao/plan.md` existe (se não existir, crie como no passo 5 de `step-01-first-run.md`). Apresente o resumo do plano e encerre a entrevista com uma única confirmação:
...
Execute o plano sequencialmente, um agente por vez, exatamente como o `solucao` faz (informar o agente, ativar o skill, salvar checkpoint, marcar ✅ no `plan.md`, resumo breve). Com estes overrides:
...
4. **Risco de violar a regra non-destructive**: qualquer situação em que prosseguir exigiria tocar arquivo fora das pastas do Solucao.
5. **Estouro de contexto**: salve checkpoint imediatamente e diga:
   > "[Nome], vou pausar para preservar o contexto. Tudo salvo. Digite `/solucao-autonomous` em uma nova sessão para continuar de onde paramos."
...
5. Sugestão de próximos passos (ex. `/solucao-forward` para evoluir o sistema, `/solucao-docs` para documentação viva).
