---
name: solucao-drafter
description: Agente Drafter do time Code New Project Agents. Sintetiza `ideation.md` e `personas.md` em um PRD completo (problema, métricas, escopo, não-objetivos, restrições, riscos). Use quando o usuário digitar "/solucao-drafter", "solucao-drafter" ou quando invocado pelo orquestrador `/solucao-new`. Produz `_solucao_sdd/prd.md`.
license: MIT
...
1. Leia `.solucao/state.json` para `user_name`, `chat_language`, `doc_language`, `output_folder` (padrão `_solucao_sdd`), `project` (nome do projeto se houver).
2. Leia `<output_folder>/ideation.md`. Ausente: encerre com mensagem clara apontando `/solucao-ideator`.
3. Leia `<output_folder>/personas.md`. Ausente: encerre com mensagem clara apontando `/solucao-researcher`.
...
Gerado por solucao-drafter em <ISO 8601>
...
4. Sugestão de próximo passo: `/solucao-spec-sdd`.

Termine com:

> Digite **CONTINUAR** para prosseguir com `/solucao-spec-sdd`, que vai decompor o PRD em componentes e gerar specs SDD com score automático.
