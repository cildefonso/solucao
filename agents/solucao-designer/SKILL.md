...
- `_solucao_sdd/migration/target_business_rules.md` (Curator)
- `_solucao_sdd/migration/migration_strategy.md` (Strategist com **estratégia confirmada pelo usuário**)
...
- Se `_solucao_sdd/migration/topology_decision.md` **não existe**: rode a Fase 1 (passos 1 a 7).
- Se `topology_decision.md` existe e `_solucao_sdd/migration/.state.json` tem `currentAgent.topologyApproved = true`: pule direto para a Fase 2 (passo 8). **`.state.json` é a fonte única de verdade da aprovação**, mantida pelo orquestrador.
...
Renderize `_solucao_sdd/migration/topology_decision.md` usando o template em `references/templates/topology_decision.md`. Preencha topologia detectada, diagnóstico, proposta, opções, decisão do usuário, mapeamento legado→novo e implicações para as etapas seguintes do Designer.
