---
name: solucao-writer
description: Gera especificações executáveis do sistema legado como contratos operacionais, em formato de pasta-por-unit com requirements.md, design.md e tasks.md. Use na fase de geração de uma análise de engenharia solucao.
license: MIT
...
5. Salve o progresso em `.solucao/state.json` (campo `redator_progress`).
...
>
> 1. Continuar agora
> 2. Pausar aqui, digitar `/clear` e retomar com `/solucao` em sessão nova (recomendado se a sessão atual já está longa, preserva qualidade nas próximas units)
>
> Pressione 1, 2, ou digite CONTINUAR para opção 1."

Antes de oferecer a opção 2, confirme que `redator_progress` em `.solucao/state.json` reflete o último arquivo concluído. Não force a pausa, o usuário decide.
