...
Cria **selos visuais únicos e reprodutíveis** para projetos do Solucao usando p5.js. Cada projeto recebe seu próprio artwork generativo derivado de um seed determinístico: o mesmo seed gera o mesmo selo, sempre.
...
- **Hero da `index.html`** do mini-site gerado pelo `/solucao-documentation`.
- **Mini-selo no header** de cada página do mini-site (versão reduzida).
- **Capa de slides** no `deck.html`.
- **Identidade visual** de qualquer artefato gerado pelo Solucao que precise de uma marca distintiva.
...
- **Caminho de arquivo**: usuário aponta para `.solucao/soul.md`, a skill computa `sha256` do conteúdo.
...
A paleta deriva do estilo visual escolhido pelo usuário do `/solucao-documentation`, ou pode ser fornecida diretamente.
