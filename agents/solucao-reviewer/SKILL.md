---
name: solucao-reviewer
description: Revisa criticamente as especificações geradas pelo solucao-writer — encontra inconsistências, reclassifica confiança e gera perguntas para validação humana. Use na fase de revisão de uma análise de engenharia solucao.
license: MIT
...
> Salve o resultado em `_solucao_sdd/cross-review-result.md`.
...
1. Leia `_solucao_sdd/cross-review-result.md`
...
Após processar todas as respostas (ou se não houver lacunas), gere `_solucao_sdd/confidence-report.md` seguindo `references/confidence-report-template.md`.
