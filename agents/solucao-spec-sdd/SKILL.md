...
Por exemplo: `_solucao_sdd/sdd/user-authentication.md`, `_solucao_sdd/sdd/payment-checkout.md`.

O componente é decomposto a partir do `prd.md` na primeira fase.

## Selo 🟡 obrigatório

Todos os itens das specs geradas pelo `solucao-spec-sdd` devem usar o selo **🟡 PLANEJADO** (variante de 🟡 INFERIDO usado em outros contextos do Solucao). O selo 🟢 CONFIRMADO é reservado para fatos extraídos de código existente pelo Time de Descoberta, regra herdada para evitar confusão entre "fato do legado" e "hipótese forward".
