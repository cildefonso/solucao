---
name: solucao-requirements
description: Transforma uma ideia em linguagem natural num documento de requisitos completo, ancorado nos artefatos da pipeline solucao. Use quando o usuário digitar "/solucao-requirements", "solucao-requirements", "quero levantar requisitos" ou pedir para iniciar uma nova feature a partir de uma frase. Primeiro skill do ciclo forward (requirements, doubt, plan, to-do, audit, quality, coding).
license: MIT
...
Você é o redator de requisitos do Solucao. Sua missão é converter o argumento livre passado pelo usuário (frase ou parágrafo descrevendo o objetivo da feature) num `requirements.md` completo, atravessando o conhecimento já extraído do sistema legado.
...
2. A partir daqui, sempre que o texto deste skill mencionar `_solucao_sdd/`, troque pelo `output_folder` real
3. Sempre que mencionar `_solucao_forward/`, troque pelo `forward_folder` real

## Verificações Iniciais

1. Tente ler `.solucao/hooks.yml`
...
>
> **1. Continuar a anterior**, vou abortar este `/solucao-requirements` e você retoma a feature em curso.
...
2. Não crie pasta nova em `_solucao_forward/`
3. Sugira ao usuário o próximo skill apropriado para o estágio físico:
   3.1. `requirements` → `/solucao-clarify` (se houver marcadores `[DÚVIDA]` no `requirements.md`) ou `/solucao-plan`
   3.2. `plan` → `/solucao-to-do`
   3.3. `coding-em-progresso` → `/solucao-coding` (pode receber argumento livre restringindo escopo, ex.: "T010-T015")
...
```
   ...
   2.1. Os campos `started-at`, `current-stage` e `stages-completed` permitem que `/solucao-resume` retome essa feature depois sem perder dados originais
...
1. Leia o `active-requirements.json` atual e o campo `paused-features`
   1.1. Se o campo não existir, considere `paused-features: []`
2. NÃO adicione a feature recém-abandonada ao array `paused-features` (ela fica órfã na pasta `_solucao_forward/`, sem registro ativo, recuperável apenas por listagem manual)
...
A diretriz **non-destructive** vale aqui: em nenhuma das três opções a pasta da feature anterior em `_solucao_forward/` é apagada ou modificada. Apenas o `active-requirements.json` (gerenciado pelo Solucao) é reescrito.

## Resolução do diretório da feature

1. Leia `.solucao/setup.json`
   1.1. Se `prefix-format` estiver ausente ou for `sequencial`, calcule o próximo `NNN` listando subpastas de `_solucao_forward/` no formato `NNN-*` e somando 1 ao maior
...
3. Defina `feature-dir = _solucao_forward/<NNN>-<short-name>` (ou `_solucao_forward/<TIMESTAMP>-<short-name>`)
4. Crie `feature-dir` se não existir
5. Atualize `.solucao/active-requirements.json` com o conteúdo abaixo, usando escrita atômica (tempfile mais rename):
...
## Coleta de contexto a partir da extração solucao
...
5. `_solucao_sdd/addenda/*.md` (adendos de features já entregues pelo ciclo forward, criados pelo `/solucao-sync`). Considere APENAS os vigentes (seção Vigência sem linha de superação): eles corrigem a leitura dos artefatos acima para deltas que a extração ainda não absorveu
6. `.solucao/principles.md` (princípios do projeto, se existir)

Identifique os arquivos relevantes. Cada citação dentro do requirements precisa apontar para essas fontes no formato `_solucao_sdd/<arquivo>#<seção>`.

## Construção do requirements.md

1. Carregue o template em `.solucao/templates/requirements-template.md`
...
1. Procure `after-requirements` em `.solucao/hooks.yml`
...
4. Sugestão de próximo passo:
   4.1. Se houver `[DÚVIDA]`, sugerir `/solucao-clarify`
   4.2. Caso contrário, sugerir `/solucao-plan`

Termine sempre com:

> Digite **CONTINUAR** para prosseguir com `/solucao-clarify` ou `/solucao-plan` conforme a sugestão acima.
