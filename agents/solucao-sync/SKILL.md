---
name: solucao-sync
description: Convergência pós-coding do Solucao. Destila a feature entregue (requirements, legacy-impact, regression-watch) em um adendo dentro de `_solucao_sdd/addenda/`, mantendo a extração solucao representativa do sistema entre re-extrações, sem tocar nos artefatos originais. Use quando o usuário digitar "/solucao-sync", "solucao-sync", "sincronizar specs", "converger a feature na extração" ou pedir para atualizar a extração com a feature recém-codada. Passo opcional do ciclo forward, depois de `/solucao-coding`.
license: MIT
...
2. Verifique a existência de `feature-dir/legacy-impact.md`
   2.1. Se ausente, aborte: "A feature ativa ainda não passou pelo `/solucao-coding`, não há entrega para converger. Rode `/solucao-coding` primeiro."
...
4. Se `feature-dir/actions.md` ainda tiver ações `[ ]` abertas, apresente o menu antes de prosseguir:
   ...
   A feature ativa ainda tem <N> ação(ões) aberta(s) em actions.md.
     ...
     [2] Aguardar: encerrar agora e voltar depois que /solucao-coding fechar todas as ações
...
Caminho: `_solucao_sdd/addenda/<feature-id>-<short-name>.md` (mesmo nome da pasta da feature em `_solucao_forward/`). Crie a pasta `addenda/` se ainda não existir.
...
4. Seção `## Impacto por artefato da extração`: tabela `Artefato | Seção | Tipo de impacto | Delta`
   4.1. **Cenário legado:** derive as linhas do `legacy-impact.md`. Componentes apontam para `_solucao_sdd/architecture.md#<seção>`, regras de negócio para `_solucao_sdd/domain.md#<seção>`. Reuse a taxonomia do coding: `regra-alterada`, `regra-removida`, `regra-nova`, `componente-novo`, `componente-extinto`, `delta-de-dados`, `delta-de-contrato-externo`
...
4. Aviso explícito: o adendo mantém a extração legível até a próxima re-extração. Ao rodar `/solucao` de novo, a verificação de regressão marcará este adendo como superado e a extração regenerada volta a ser a fonte única

Termine com:

> Digite **CONTINUAR** para prosseguir com `/solucao-forward` (nova feature) ou digite `/solucao` quando quiser a re-extração completa.

## Regra absoluta

**Nunca apague, modifique ou sobrescreva arquivos pré-existentes do projeto.**
Este skill escreve APENAS em `_solucao_sdd/addenda/`. Os artefatos originais da extração e os artefatos da feature em `_solucao_forward/` são somente leitura aqui.
