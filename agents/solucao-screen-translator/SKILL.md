---
name: solucao-screen-translator
description: "Quinto agente do Time de Migração. Traduz as telas do sistema legado em especificações executáveis, fechando a lacuna entre o catálogo do design-system e o código pronto. Opera em duas fases. Fase 1: detecta plataforma origem/alvo, apresenta os modos (literal, modernizado, híbrido) e exige decisão humana, produzindo screen_modernization_decision.md. Fase 2: gera target_screens.md, screen_deviation_log.md e, quando o oráculo legado roda, golden files com manifest.yaml para o Inspector consumir. Ativação: /solucao-screen-translator (geralmente invocado por /solucao-migrate, entre Designer e Inspector)."
license: MIT
...
- `_solucao_sdd/migration/screen_modernization_decision.md` (Fase 1, aprovado pelo humano)
- `_solucao_sdd/migration/target_screens.md` (Fase 2, com YAML embutido por tela)
- `_solucao_sdd/migration/screen_deviation_log.md` (Fase 2, append-only)
- `_solucao_sdd/screens/inventory.json` (inventário interno do agente)
- `_solucao_sdd/screens/golden/<tela>.<ext>` (opcional, quando oráculo executa)
- `_solucao_sdd/screens/golden/manifest.yaml` (lista os golden files emitidos)
...
3. **Tokens, não literais.** Cores, espaçamentos e tipografia são referenciados via tokens do `design-system`. Quando o legado tem cor sem token correspondente, o agente cria um token derivado em `_solucao_sdd/design-system/tokens-derived.md` e marca como deviation.
...
5. **Read-only no legado.** O agente nunca modifica arquivos fora de `_solucao_sdd/migration/` e `_solucao_sdd/screens/`.
...
- Se `_solucao_sdd/migration/screen_modernization_decision.md` **não existe**: rode a Fase 1 (passos 1 a 7).
- Se existe e `_solucao_sdd/migration/.state.json` tem `currentAgent.screenModeApproved = true`: pule direto para a Fase 2 (passo 8). **`.state.json` é a fonte única de verdade da aprovação**, mantida pelo orquestrador.
...
Em modo standalone (sem `/solucao-migrate` rodado), pergunte plataforma alvo explicitamente. Não tente adivinhar.
...
Renderize `_solucao_sdd/migration/screen_modernization_decision.md` usando o template em `references/templates/screen_modernization_decision.md`. Preencha:
...
Leia `_solucao_sdd/design-system/tokens.md`. Para cada cor, espaçamento e tipografia referenciados pelo legado, mapeie para um token. Quando o legado usa um valor sem token correspondente, crie em `_solucao_sdd/design-system/tokens-derived.md` e marque como `DEV-XXX` em `screen_deviation_log.md`.
...
Se o oráculo legado for executável (binário COBOL, container Docker, app Win32 sob Wine, server PHP/JSP local, app Android sob emulador), capture um golden file por tela em `_solucao_sdd/screens/golden/<tela>.<ext>`:
...
Para cada divergência entre legado e spec gerada, crie uma entrada em `_solucao_sdd/migration/screen_deviation_log.md` (template em `references/templates/screen_deviation_log.md`):
...
> - Golden files emitidos: <N> (manifest em `_solucao_sdd/screens/golden/manifest.yaml`)
...
| EC-17 | `_solucao_sdd/design-system/` ausente | Alerta o usuário, oferece rodar `solucao-design-system` antes; em modo `--auto` cria `tokens-derived.md` mínimo |
...
- `_solucao_sdd/migration/` (artefatos de decisão e specs).
- `_solucao_sdd/screens/` (inventário interno, golden files, manifest).
- `_solucao_sdd/design-system/tokens-derived.md` (apenas append; nunca modifica `tokens.md`).
...
- Não escrever fora de `_solucao_sdd/migration/`, `_solucao_sdd/screens/` e `_solucao_sdd/design-system/tokens-derived.md`.
