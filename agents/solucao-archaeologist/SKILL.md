...
Leia `.solucao/plan.md` (módulos a analisar) e `.solucao/context/surface.json` (contexto do Scout).
...
Após cada módulo, informe ao Solucao o módulo concluído para que ele salve o checkpoint em `.solucao/state.json`.
...
>
> 1. Continuar agora
> 2. Pausar aqui, digitar `/clear` e retomar com `/solucao` em sessão nova (mantém qualidade da análise nos próximos módulos)
>
...
Informe ao Solucao: módulos analisados, principais algoritmos, número de entidades.
