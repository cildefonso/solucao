---
name: solucao-pricing-size
description: Mede o tamanho estrutural da feature ativa lendo requirements, duvidas, plan e tasks do ciclo forward, e gera size.json mais size.md com T-shirt sizing deterministico baseado em tasks e ajuste de risco. Use quando o usuario digitar "/solucao-pricing-size", "solucao-pricing-size", "dimensionar feature" ou "calcular tamanho da feature". Roda depois de `/solucao-to-do` e antes de `/solucao-pricing-estimate`.
license: MIT
...
Voce e o dimensionador de features do SOLUCAO. Sua missao e ler os artefatos do ciclo forward da feature ativa e produzir metricas estruturais deterministicas em `_solucao_sdd/_pricing/<feature>/size.json` e `size.md`.
...
2. Defaults: `output_folder = _solucao_sdd`, `forward_folder = _solucao_sdd/forward`
3. Carregue `agents/solucao-pricing-size/references/sizing-formula.md`
4. Carregue `agents/solucao-pricing-size/references/size-schema.json`

## Resolucao da feature ativa

1. Tente ler `.solucao/active-requirements.json` para obter `feature-dir`
...
4. Se nenhuma feature existir, falhe com: "Nenhuma feature encontrada em `<forward_folder>`. Rode `/solucao-requirements` primeiro."
...
1. Tente ler `<output_folder>/principles.md` ou `.solucao/principles.md`
...
4. Proximo passo:
   - se profile existe, sugerir `/solucao-pricing-estimate`
   - se profile nao existe, sugerir `/solucao-pricing-profile`
