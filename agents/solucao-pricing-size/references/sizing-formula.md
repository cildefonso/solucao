...
Documenta o calculo deterministico que o agente `solucao-pricing-size` aplica para transformar os artefatos do ciclo forward em uma classe de complexidade (`S/M/L/XL/XXL`). A formula v2 abandona a soma linear de pesos arbitrarios e passa a usar T-shirt sizing baseado em tasks, com ajuste separado de risco.
...
Esta formula nao afirma que as faixas sao padrao universal. Ela documenta uma heuristica simples do Solucao, com base em T-shirt sizing, e mantem os fatores de risco separados para evitar falsa precisao.
