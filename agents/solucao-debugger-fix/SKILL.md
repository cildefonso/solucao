---
name: solucao-debugger-fix
description: Corretor de bugs do Solucao, orquestrador do ciclo de vida do defeito. Reproduz, investiga causa raiz com evidências, oferece debate multiagente opt-in, cria testes de reprodução e regressão, aplica o Correction Change Set com diffs aprovados em dois gates, dá o veredito de spec (com adendo versionado quando a spec muda) e fecha conforme a closure policy. Use quando o usuário digitar "/solucao-debugger-fix", "solucao-debugger-fix", "corrigir o bug", "consertar o BUG-XXX" ou pedir a correção de um bug registrado. Exige bug registrado via `/solucao-debugger`.
license: MIT
...
1. Leia `.solucao/state.json` (`output_folder`, `chat_language`, `doc_language`, `user_name`)
2. Leia `_solucao_bugs/README.md` (closure policy, control_mode) e o schema em `references/../solucao-debugger/references/bug-schema.md` se disponível; senão siga o contrato descrito no README do registro
3. Se `_solucao_bugs/` não existir, aborte: "Não há registro de bugs neste projeto. Rode `/solucao-debugger` primeiro."

## Seleção do bug

1. Com argumento (`/solucao-debugger-fix BUG-20260715-A7K3` ou `/solucao-debugger-fix BUG-007`): resolva por ID canônico ou `display_number`
2. O bug vive em `_solucao_bugs/<contexto>/bugs/`: localize-o varrendo os catálogos de todos os contextos (`_solucao_bugs/*/generated/catalog.jsonl`, ou `_solucao_bugs/*/bugs/*/bug.md` na falta deles). Se o usuário falou da área em linguagem natural ("conserta o carrinho"), comece pelo contexto correspondente.
...
Causa raiz: <resumo> (estado: <state>). Risco da mudança: <classificação> (<motivos>).
  ...
  [2] Debate multiagente
      /solucao-debugger-debate em modo <diagnosis|repair> com N agentes por R rodadas + juiz.
...
2. `spec-desatualizada`: o comportamento correto mudou ou a spec descrevia errado. Gere adendo versionado e imutável `_solucao_sdd/addenda/bug-<ID>-vNNN.md` com: seção alvo, delta (trecho antes / como deve ser lido agora), vigência, evidências, aprovação registrada. A spec original NUNCA é editada. O adendo entra no change set como `kind: specification`.
...
5. Atualize as views do contexto do bug (`_solucao_bugs/<contexto>/generated/`) e o espelho `_solucao_sdd/traceability/bugs.md` pelo protocolo do `/solucao-debugger-graph`
...
> Digite **CONTINUAR** para atualizar as views com `/solucao-debugger-graph`, corrigir o próximo bug com `/solucao-debugger-fix`, ou encerrar.

## Regra absoluta

**Nunca apague, modifique ou sobrescreva arquivos pré-existentes do projeto sem gate aprovado.**
Fora dos dois gates (e do reparo de dados aprovado), este skill escreve apenas em `_solucao_bugs/` e em `_solucao_sdd/addenda/` + `_solucao_sdd/traceability/`. Specs originais são somente leitura para sempre. Bug com `visibility: restricted`: nenhum detalhe explorável sai do registro.
