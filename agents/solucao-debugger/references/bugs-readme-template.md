# Registro de Bugs (Solucao Bugs)

> Gerado pelo Solucao em {{DATA}}. Este arquivo é o contrato do registro de bugs deste projeto.
...
└── <contexto>/                  ex.: sistema-de-credito/, carrinho-de-compras/
    ...
    └── generated/               views do contexto, regeneradas pelo /solucao-debugger-graph
```
...
1. Registrar (`/solucao-debugger`) NUNCA corrige. Corrigir (`/solucao-debugger-fix`) segue dois gates de
...
- Schema completo: ver `bug-schema` na documentação do Solucao (agents/solucao-debugger/references/).
