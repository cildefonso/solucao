---
name: solucao-depth-inspection
description: Pente-fino do Time Solucao Bugs. Dada uma feature problemática, monta o mapa spec→código→testes→dados e varre com lentes especializadas (conformidade com spec, fluxo de dados, contratos, estados de erro, cobertura de testes, concorrência), em subagentes paralelos quando o harness suportar. SÓ diagnostica: achados confirmados viram bugs registrados com rastreabilidade; nada é corrigido. Use quando o usuário digitar "/solucao-depth-inspection", "solucao-depth-inspection", "pente-fino na feature", "inspeção profunda", "essa feature vive dando problema" ou pedir varredura completa de uma área problemática.
license: MIT
...
1. Leia `.solucao/state.json` (`output_folder`, `chat_language`, `doc_language`)
2. Se `_solucao_bugs/` não existir, execute o bootstrap do registro descrito no `/solucao-debugger` (APENAS README com closure policy e taxonomy.yaml; nenhuma pasta vazia)
2.1. Resolva o **contexto** (pasta agregadora da feature/módulo/caso de uso) como no `/solucao-debugger`: case a fala do usuário com as pastas de contexto existentes em `_solucao_bugs/` e com o taxonomy.yaml, confirme via menu, e só crie `_solucao_bugs/<contexto>/` quando a varredura de fato produzir artefatos
...
1. **Specs**: seções de `_solucao_sdd/` que definem a feature (spec efetiva: original + adendos vigentes)
...
4. Registre os aceitos EM SÉRIE seguindo o protocolo do `/solucao-debugger`, dentro de `_solucao_bugs/<contexto>/bugs/` (IDs merge-safe atribuídos um a um, `origin.type: inspection`, rastreabilidade e relações preenchidas). Achado com sinal de segurança segue o fluxo restrito.

## Etapa 4: relatório

Escreva `_solucao_bugs/<contexto>/inspections/<varredura>/report.md` (crie `inspections/` do contexto agora, na primeira varredura):
...
Atualize as views do contexto (`_solucao_bugs/<contexto>/generated/`, incluindo o `graph.html`) pelo protocolo do `/solucao-debugger-graph`.
...
> Digite **CONTINUAR** para corrigir o bug de maior impacto com `/solucao-debugger-fix`, ou rode `/solucao-debugger-graph` para ver o panorama.

## Regra absoluta

**Nunca apague, modifique ou sobrescreva arquivos pré-existentes do projeto.**
Este skill escreve APENAS em `_solucao_bugs/` (bugs novos, relatório e views). Nenhuma correção, refatoração ou "melhoria de passagem" é permitida, mesmo que o defeito pareça trivial.
