// Fonte única da regra de não-destrutividade do Solucao: as únicas pastas em
...
//
// `_solucao_sdd` e `_solucao_forward` são configuráveis pelo usuário
// (state.json: output_folder / forward_folder). `_solucao_docs` é fixo por
...
// só conhecem `.solucao/` + output_folder e divergem da regra global.
export function getWritableFolders({
  outputFolder = '_solucao_sdd',
  forwardFolder = '_solucao_forward',
} = {}) {
  return ['.solucao/', `${outputFolder}/`, '_solucao_docs/', `${forwardFolder}/`];
}
...
export function renderPolicyBlock(opts = {}) {
  ...
  return [
    ...
    `O Solucao escreve apenas em ${list}.`,
  ].join('\n');
}
