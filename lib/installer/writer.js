...
export class Writer {
  ...
  async installEntryFile(engine, { force = false, outputFolder, forwardFolder } = {}) {
    ...
    const content = readFileSync(templatePath, 'utf8')
      .replace('{{SOLUCAO_POLICY}}', renderPolicyBlock({ outputFolder, forwardFolder }));
  ...
  // Cria a estrutura interna .solucao/
  createSolucaoDir(answers, version) {
    const solucaoDir = join(this.projectRoot, '.solucao');
    const configDir = join(solucaoDir, '_config');

    this._mkdir(solucaoDir);
    this._mkdir(configDir);
    this._mkdir(join(solucaoDir, 'context'));

    // Estrutura forward (templates de corpo, scripts, hooks, setup)
    this._installForwardAssets(solucaoDir, answers, version);
    ...
    const statePath = join(solucaoDir, 'state.json');
    ...
    const config = configTemplate
      ...
      .replace('folder = "_solucao_sdd"', `folder = "${answers.output_folder}"`)
      .replace(
        /\[agents\]\r?\ninstalled = \[[\s\S]*?\]/,
        `[agents]\ninstalled = [\n${agentsList}\n]`
      )
    ...
    this._writeNew(join(solucaoDir, 'config.toml'), config);
    this._writeNew(join(solucaoDir, 'config.user.toml'),
    ...
    this._writeNew(join(solucaoDir, 'plan.md'), plan);

    // version
    this._writeNew(join(solucaoDir, 'version'), version);
  ...
  // Copia body templates, scripts, hooks.yml e setup.json forward para .solucao/
  // Não sobrescreve arquivos pré-existentes (preserva edits do usuário em refresh)
  _installForwardAssets(solucaoDir, answers, version) {
    const forwardSrc = join(TEMPLATES_DIR, 'forward');
    if (!existsSync(forwardSrc)) return;

    // Body templates → .solucao/templates/
    const bodySrc = join(forwardSrc, 'body');
    const bodyDest = join(solucaoDir, 'templates');
    ...
    // Scripts sh + ps → .solucao/scripts/{sh,ps}/
    for (const flavor of ['sh', 'ps']) {
      const scriptSrc = join(forwardSrc, 'scripts', flavor);
      const scriptDest = join(solucaoDir, 'scripts', flavor);
    ...
    // hooks.yml → .solucao/hooks.yml
    const hooksSrc = join(forwardSrc, 'hooks.yml');
    const hooksDest = join(solucaoDir, 'hooks.yml');
    ...
    // setup.json → .solucao/setup.json (com placeholders)
    const setupSrc = join(forwardSrc, 'setup.json');
    const setupDest = join(solucaoDir, 'setup.json');
  ...
  // Refresca body templates, scripts e hooks.yml em .solucao/ a partir do pacote npm,
  ...
  // Adiciona _solucao_sdd/ e .solucao/config.user.toml ao .gitignore
  ...
  saveCreatedFiles() {
    const statePath = join(this.projectRoot, '.solucao', 'state.json');
