...
import { runInstallPrompts, MIGRATION_AGENT_IDS, TRANSLATOR_AGENT_IDS, FORWARD_AGENT_IDS, PRICING_AGENT_IDS, DOCS_AGENT_IDS, NEW_PROJECT_AGENT_IDS, BUGS_AGENT_IDS } from '../installer/prompts.js';
import { Writer } from '../installer/writer.js';
import { buildManifest, saveManifest, loadManifest } from '../installer/manifest.js';
import { readJsonSafe } from '../utils/json-safe.js';
import { clearTerminalForLogo, renderSolucaoLogo } from '../utils/banner.js';
...
export default async function install(args) {
  ...
  if (existing.installed) {
    console.log(chalk.yellow(`  Solucao is already installed (v${existing.version}) in this project.\n`));
  ...
  try {
    ...
    spinner.start('Creating .solucao/ structure...');

    // Criar estrutura .solucao/
    writer.createSolucaoDir(answers, version);
  ...
  if (migrationInstalled.length > 0) {
    ...
    console.log(`  ${chalk.gray('Migration Team:       not installed (run')} ${chalk.cyan('npx solucao add-agent')}${chalk.gray(' to add later)')}`);
  }
  ...
  if (selectedEngines.length > 0) {
    ...
    const startCommand = hasSlashEngine ? '/solucao' : 'solucao';
    console.log(chalk.cyan(`  → Open ${namesStr} and type: ${startCommand} in the chat to start the discovery`));
    if (migrationInstalled.length > 0) {
      const migrateCommand = hasSlashEngine ? '/solucao-migrate' : 'solucao-migrate';
      console.log(chalk.cyan(`  → After discovery completes, run ${migrateCommand} to plan the rebuild`));
    }
    if (docsInstalled.length > 0) {
      const docsCommand = hasSlashEngine ? '/solucao-docs' : 'solucao-docs';
      console.log(chalk.cyan(`  → For a visual HTML mini-site of the project, run ${docsCommand} after discovery`));
    }
    if (newProjectInstalled.length > 0) {
      const newCommand = hasSlashEngine ? '/solucao-new' : 'solucao-new';
      console.log(chalk.cyan(`  → For a brand-new project from idea to specs, run ${newCommand}`));
    }
    if (bugsInstalled.length > 0) {
      const bugCommand = hasSlashEngine ? '/solucao-debugger' : 'solucao-debugger';
      const bugFixCommand = hasSlashEngine ? '/solucao-debugger-fix' : 'solucao-debugger-fix';
      console.log(chalk.cyan(`  → To track a defect, run ${bugCommand}; to fix a registered one, run ${bugFixCommand}`));
    }
    if (translatorsInstalled.includes('solucao-n8n')) {
      const n8nCommand = hasSlashEngine ? '/solucao-n8n' : 'solucao-n8n';
      console.log(chalk.cyan(`  → To analyze N8N workflows, drop the JSONs in n8n_json_workflows/ and run ${n8nCommand}`));
    }
    if (pricingInstalled.length > 0) {
      const profileCommand = hasSlashEngine ? '/solucao-pricing-profile' : 'solucao-pricing-profile';
      const sizeCommand = hasSlashEngine ? '/solucao-pricing-size' : 'solucao-pricing-size';
      const estimateCommand = hasSlashEngine ? '/solucao-pricing-estimate' : 'solucao-pricing-estimate';
      console.log(chalk.cyan(`  → To set up pricing, run ${profileCommand} once per project, then ${sizeCommand} after each /solucao-to-do, then ${estimateCommand} to get the 3 price scenarios`));
    }
  }
  console.log('');
}
