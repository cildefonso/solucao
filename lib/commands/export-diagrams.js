...
export default async function exportDiagrams(args) {
  ...
  if (!mmdc) {
    ...
    console.log('  Then run: ' + chalk.bold('npx solucao export-diagrams\n'));
    process.exit(1);
  }

  const statePath = join(process.cwd(), '.solucao', 'state.json');
  if (!existsSync(statePath)) {
    console.log(chalk.yellow('\n  Solucao is not installed in this directory.'));
    console.log('  Run ' + chalk.bold('npx solucao install') + ' to install.\n');
    return;
  }
  ...
  for (const { diagram, source, index } of blocks) {
    ...
    const tmpFile = join(tmpdir(), `solucao-${Date.now()}-${index}.mmd`);
