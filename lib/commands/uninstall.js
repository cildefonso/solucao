...
export default async function uninstall(args) {
  ...
  const outputFolder = state.output_folder ?? '_solucao_sdd';
  ...
  // Remover .solucao/ por inteiro
  const solucaoDir = join(projectRoot, '.solucao');
  try {
    if (existsSync(solucaoDir)) {
      rmSync(solucaoDir, { recursive: true, force: true });
      removed++;
    }
  } catch {
    console.error(chalk.red('    Error removing .solucao/'));
    errors++;
  }
  ...
  console.log('');
  if (errors === 0) {
    console.log(chalk.hex('#ffa203')('  Solucao removed successfully.\n'));
  } else {
