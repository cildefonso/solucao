/*
 * Solucao Documentation - Helper de sidebar reativa
 *
 * Cada controle dentro da sidebar declara um data-param.
 * Mudanças disparam o evento "solucao:param-change" para
...
(function () {
    ...
    const STORAGE_PREFIX = "solucao.sidebar.";

    /**
     * Inicializa todos os controles da sidebar atual.
     */
    function init() {
        const sidebar = document.querySelector(".solucao-doc-sidebar");
        if (!sidebar) return;
    ...
    function dispatchChange(param, value, control) {
        const event = new CustomEvent("solucao:param-change", {
    ...
    function inferExportName() {
        ...
        return "solucao-" + page + "-" + stamp + ".png";
    }

    function showToast(message) {
        const existing = document.querySelector(".solucao-toast");
        if (existing) existing.remove();
        const toast = document.createElement("div");
        toast.className = "solucao-toast";
